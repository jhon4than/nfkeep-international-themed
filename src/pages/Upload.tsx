...
Se alguma informação não estiver disponível, deixe o campo vazio (""). Retorne APENAS o JSON válido.`;
      ...
      const extractInvoiceData = (data: any): InvoiceForm => {
        ...
        const firstItem =
          Array.isArray(payload.itens) && payload.itens.length > 0
            ...
            : null;
        const itemDesc =
          payload.item_description ?? firstItem?.description ?? "";
        const itemQty = payload.item_quantity ?? firstItem?.quantity ?? null;
        const itemUnit =
          payload.item_unit_price ?? firstItem?.unit_price ?? "0";
        const itemTotal =
          payload.item_line_total ??
          firstItem?.line_total ??
          payload.total_amount ??
          "0";
        const kindRaw = (payload.kind ?? "nfe").toLowerCase();
        const kind: KindType =
          kindRaw === "nfce" ? "nfce" : kindRaw === "sat" ? "sat" : "nfe";

        const warrantyUnitRaw = String(
          payload.warranty_unit ?? "mes"
        ).toLowerCase();
        ...
        return {
          ...
          item_quantity:
            itemQty === null || itemQty === undefined ? null : String(itemQty),
  ...
  const saveInvoice = async () => {
    try {
      ...
      const warranty_days = warrantyToDays(
        invoiceForm.warranty_value,
        invoiceForm.warranty_unit
      );
      // Fallback enquanto a migração não está aplicada: salvar em meses
      const warranty_months =
        warranty_days == null ? null : Math.ceil(warranty_days / 30);
      ...
      const proceedSave = async () => {
        ...
        const insertRes = await supabase
          ...
          .insert({
            ...
            item_quantity:
              invoiceForm.item_quantity !== undefined &&
              invoiceForm.item_quantity !== null &&
              String(invoiceForm.item_quantity).trim() !== ""
                ? Number(invoiceForm.item_quantity)
                : null,
            item_unit_price:
              invoiceForm.item_unit_price !== undefined &&
              invoiceForm.item_unit_price !== null &&
              String(invoiceForm.item_unit_price).trim() !== ""
                ? Number(invoiceForm.item_unit_price)
                : null,
            item_line_total:
              invoiceForm.item_line_total !== undefined &&
              invoiceForm.item_line_total !== null &&
              String(invoiceForm.item_line_total).trim() !== ""
        ...
        <div className="space-y-4 sm:space-y-6">
          <div>
            ...
            <p className="text-sm sm:text-base text-muted-foreground mt-1">
              Envie um arquivo (imagem, PDF ou XML) para processarmos os dados
              da sua nota.
            </p>
          </div>
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Selecione ou arraste o arquivo da sua nota fiscal para
                processar.
              </CardDescription>
            </CardHeader>
                      ...
                      {selectedFile.type.startsWith("image/") && previewUrl ? (
                        ...
                        <div className="w-28 h-28 sm:w-32 sm:h-32 bg-red-50 border-2 border-red-200 rounded-lg flex items-center justify-center mx-auto">
                          <div className="text-center">
                            <div className="text-red-600 text-xl sm:text-2xl mb-2">
                              📄
                            </div>
                        ...
                        <div className="w-28 h-28 sm:w-32 sm:h-32 bg-blue-50 border-2 border-blue-200 rounded-lg flex items-center justify-center mx-auto">
                          <div className="text-center">
                            <div className="text-blue-600 text-xl sm:text-2xl mb-2">
                              📋
                            </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        <p className="text-lg font-medium">
                          Arraste um arquivo (imagem, PDF ou XML)
                        </p>
                        <p className="text-sm text-muted-foreground">
              ...
              if (!next) {
                ...
                if (formDirty) {
                  ...
                  const id = toast.warning("Deseja sair", {
                    description:
                      "Se sair, os dados do formulário serão perdidos.",
                    duration: Infinity,
                    action: {
                      label: "Sair",
                      onClick: () => {
                        ...
                        if (exitToastId !== null)
                          toast.dismiss(exitToastId as any);
                        setExitToastId(null);
                      },
                    },
                    cancel: {
            ...
            >
              <DialogHeader>
                ...
                <DialogDescription className="text-sm text-muted-foreground">
                  Revise os campos e ajuste antes de salvar
                </DialogDescription>
              </DialogHeader>
              <div className="space-y-4">
                {/* Garantia */}
                <div className="sm:col-span-2">
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-2">
                    <Input
                      ...
                      onChange={(e) =>
                        updateForm({ warranty_value: e.target.value })
                      }
                    />
                    <Select
                      value={invoiceForm.warranty_unit}
                      onValueChange={(v) =>
                        updateForm({ warranty_unit: v as WarrantyUnit })
                      }
                    >
                      <SelectTrigger>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="sm:col-span-2">
                    ...
                    <Input
                      value={invoiceForm.access_key}
                      onChange={(e) =>
                        updateForm({ access_key: e.target.value })
                      }
                    />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        updateForm({ issue_date: e.target.value })
                      }
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        updateForm({ total_amount: e.target.value })
                      }
                    />
                  </div>
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(v) =>
                        updateForm({ kind: v.toLowerCase() as KindType })
                      }
                    >
                      <SelectTrigger>
                  ...
                  <div>
                    ...
                    <Input
                      value={invoiceForm.emitente_cnpj}
                      onChange={(e) =>
                        updateForm({ emitente_cnpj: e.target.value })
                      }
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      value={invoiceForm.emitente_name}
                      onChange={(e) =>
                        updateForm({ emitente_name: e.target.value })
                      }
                    />
                  </div>
                  <div className="sm:col-span-2">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        updateForm({ item_quantity: e.target.value || null })
                      }
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        updateForm({ item_unit_price: e.target.value })
                      }
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      value={invoiceForm.item_line_total}
                      onChange={(e) =>
                        updateForm({ item_line_total: e.target.value })
                      }
                    />
                  </div>
                </div>
