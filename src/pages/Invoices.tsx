...
import { InvoiceWarrantyBadge } from "@/components/invoices/InvoiceWarrantyBadge";
...
export default function Invoices() {
  ...
  const getDaysToExpireWarranty = (
    warrantyEndDays: number,
    ...
    const warrantyDate = new Date(issueDateString);
    warrantyDate.setDate(issueDate.getDate() + warrantyEndDays)
    
    const now = Date.now();
    
    const timeDifference = warrantyDate.getTime() - now;
    
    const daysDifference = Math.floor(timeDifference / ONE_DAY_IN_MILLISECONDS);
              ...
              <>
                ...
                <div className="md:hidden space-y-3">
                  {filtered.map((inv) => (
                    <div
                      key={inv.id}
                      className="rounded-lg border border-slate-200 dark:border-slate-700 dark:bg-slate-800 p-4 flex flex-col gap-3"
                    >
                          ...
                          {previews[inv.id] ? (
                            previews[inv.id].type === "image" ? (
                              ...
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  window.open(previews[inv.id].url, "_blank")
                                }
                              >
                                PDF
                              </Button>
                    ...
                    <TableBody>
                      {filtered.map((inv) => (
                        <TableRow key={inv.id}>
                          ...
                          <TableCell>
                            <InvoiceWarrantyBadge
                              daysToExpire={getDaysToExpireWarranty(
                                inv.warranty_days,
                                inv.issue_date
                              )}
                            />
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-lg font-semibold">
                      {detailsData.issue_date
                        ? new Date(detailsData.issue_date).toLocaleDateString(
                            "pt-BR"
                          )
                        : "—"}
                    </p>
